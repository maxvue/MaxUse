...
var getVueUseTypes = () => {
	try {
		...
		const types = allExports.filter((name) => !valueKeys.includes(name)).map((name) => [name, `type ${name}`]);
		const sharedDtsPath = path.resolve(process.cwd(), "node_modules/@vueuse/shared/dist/index.d.ts");
		if (fs.existsSync(sharedDtsPath)) {
			const sharedExportMatch = fs.readFileSync(sharedDtsPath, "utf-8").match(/export\s*\{([^}]+)\}/g);
			if (sharedExportMatch) {
				const sharedTypes = sharedExportMatch[sharedExportMatch.length - 1].replace(/export\s*\{|\}/g, "").split(",").map((s) => s.trim()).filter(Boolean).filter((name) => !valueKeys.includes(name)).map((name) => [name, `type ${name}`]);
