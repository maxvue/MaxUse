...
**Goal:** Produzir o conjunto de artefatos `lodash_migrate/` (279 planos individuais + `execution.md` + `status.yaml` + docs de apoio) e o scaffolding de código (4 categorias novas + correção de precedência), de modo que uma sessão separada do Claude Code consiga executar a migração em loop até eliminar o `lodash-es`.

**Architecture:** Este plano **não implementa os 279 helpers**. Ele constrói: (a) o scaffolding das 4 categorias novas registrado nos 5 pontos de agregação; (b) a correção do bug de precedência em `src/index.ts`; (c) um gerador determinístico que produz os 279 arquivos `.md` de plano a partir de um manifesto categorizado versionado; (d) os documentos de controle (`execution.md`, `status.yaml`, `CONVENTIONS.md`, `DIVERGENCES.md`). A execução dos 279 helpers acontece depois, numa sessão separada, dirigida por `execution.md`.
...
- **Total imutável:** são exatamente **279** helpers. Qualquer divergência desse número indica erro e deve travar a geração.
...
|---|---|
| `lodash_migrate/manifest.ts` | Fonte da verdade: os 279 helpers com categoria, fase, dependências, aliases e notas de peculiaridade. |
| `lodash_migrate/generate.ts` | Gerador determinístico: lê `manifest.ts` → emite os 279 `plans/**/*.md` e o `status.yaml`. |
| `lodash_migrate/CONVENTIONS.md` | Contrato de helper e de teste (evita repetir em 279 arquivos). |
...
| `lodash_migrate/status.yaml` | Estado de execução/verificação dos 279 (gerado). |
| `lodash_migrate/plans/<Cat>/<nome>.md` | 279 planos individuais (gerados). |
...
Cada categoria precisa de um helper real para o barrel não ser um módulo vazio (o Vite falha em entry sem exports). Uso `stubTrue`, `isNil`, `constant` e `tap` como sementes: são triviais, sem dependências, e já fazem parte dos 279.
...
### Task 3: Manifesto dos 279 helpers

O manifesto é a fonte da verdade da migração: define categoria, fase, dependências, aliases e peculiaridades de cada helper. Já foi construído e **validado** durante o planejamento (279 exatos, zero lacunas, zero extras, nenhuma dependência inválida ou apontando para fase posterior).
...
- Produces: `HELPERS: HelperEntry[]` (279 itens) e a interface `HelperEntry` com os campos `nome`, `categoria`, `fase`, `depende_de`, `alias_de?`, `nota?`. A Task 4 consome isto para gerar planos e `status.yaml`.
...
git commit -m "feat: adiciona manifesto validado dos 279 helpers do Lodash"
```

---

### Task 4: Gerador dos 279 planos e do status.yaml

Escrever 279 arquivos `.md` à mão é inviável e propenso a inconsistência. O gerador os produz a partir do manifesto, garantindo formato uniforme. Cada plano é curto e específico — o contrato comum vive em `CONVENTIONS.md` (Task 5), não repetido 279 vezes.

**Files:**
- Create: `lodash_migrate/generate.ts`
- Create: `lodash_migrate/plans/<Categoria>/<nome>.md` (279, gerados)
...
- Produces: `lodash_migrate/plans/**/*.md` (279 arquivos) e `lodash_migrate/status.yaml` com a chave `helpers[]` (campos `nome`, `categoria`, `fase`, `plano`, `depende_de`, `tentativas`, `status_execucao`, `status_verificacao`) mais a chave `fases[]`. A Task 6 (`execution.md`) documenta o consumo deste YAML.
...
Expected: `Gerados 279 planos em lodash_migrate/plans e o status.yaml`
...
if (s.helpers.length !== 279) throw new Error('status.yaml com ' + s.helpers.length);
...
console.log('OK — 279 planos, status.yaml íntegro, todos os caminhos resolvem');
"
```

Expected: `279` seguido de `OK — 279 planos, status.yaml íntegro, todos os caminhos resolvem`
...
git commit -m "feat: gera os 279 planos de migração e o status.yaml"
```
...
`CONVENTIONS.md` é o contrato que os 279 planos referenciam. `DIVERGENCES.md` documenta os 36 nomes onde a MaxUse diverge do Lodash, acompanhado de uma suite que trava essa divergência — sem ela, um agente futuro "consertaria" a divergência achando que é bug.
...
Contrato obrigatório para todos os 279 helpers da migração. Os planos individuais
...
Você vai implementar, em loop, os 279 helpers que substituem o `lodash-es` na
...
## Encerramento (só depois dos 279)
...
2. **Nunca** remova o `lodash-es` antes dos 279 concluídos — ele é a rede de
...
Expected: mudanças em `src/index.ts`, `src/Helpers/maxUseItems.ts`, `src/scripts/buildAutoImport.ts`, `vite.config.ts`, `package.json`, `vitest.config.ts`; arquivos novos em `src/Helpers/{Lang,Functions,Utils,Seq}/` e `lodash_migrate/`. Nenhum helper dos 279 implementado além dos 4 semente — isso é esperado.
...
|---|---|
| Mapear os helpers lodash faltantes | Task 3 (manifesto validado, 279) |
...
**Riscos conhecidos:**
- `isNil`, `negate`, `stubTrue` e `tap` são implementados na Task 2 como sementes, mas continuam listados no manifesto. O teste da Task 3 detecta isso como "extra no manifesto" e o Step 3 instrui a removê-los. São 4 de 279 — o total efetivo a implementar na sessão de execução passa a 275.
