...
/**
 * Preenche o fim de uma string até que ela atinja `length` caracteres.
 * Semelhante ao _.padEnd do Lodash.
 *
 * @param string string a preencher
 * @param length comprimento final desejado
 * @param chars caractere(s) de preenchimento (padrão: espaço)
 * @returns string preenchida no fim
 */
export function padEnd(string: MaybeRefOrGetter<unknown>, length?: MaybeRefOrGetter<number>, chars?: MaybeRefOrGetter<string>): string {
    ...
    return str + createPadding(len - strLength, charSet);
}
