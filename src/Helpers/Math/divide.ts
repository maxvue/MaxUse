 ...
 * (`toNumber`), sem o tratamento especial de string usado em `divide`.
 *
...
/**
 * Divide `value` por `other`. Se ambos forem `undefined`, retorna `1`; se
 ...
 * Lodash: `_.divide('3', true)` vira `'3' / 'true'` = `NaN`. Divisão por
 * zero retorna `Infinity`/`-Infinity` (comportamento nativo do JS).
 * Semelhante ao _.divide do Lodash.
 *
 * @param value dividendo
 * @param other divisor
 * @returns quociente dos dois valores
 */
export function divide(value?: MaybeRefOrGetter<unknown>, other?: MaybeRefOrGetter<unknown>): number {
