...
/**
 * Arredonda `number` até `precision` casas decimais.
 * Semelhante ao _.round do Lodash.
 *
 ...
 */
export function round(number: MaybeRefOrGetter<number>, precision: MaybeRefOrGetter<number> = 0): number {
    const num = Number(toValue(number));
    const prec = toValue(precision) ?? 0;
    return createRound(num, prec, Math.round);
}
