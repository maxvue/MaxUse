 ...
 * (`toNumber`), sem o tratamento especial de string usado em `add`.
 *
...
/**
 * Soma dois valores. Se ambos forem `undefined`, retorna `0`; se apenas um
 * for fornecido, retorna esse valor sem operação. Se algum dos dois for
 * string, concatena em vez de somar numericamente.
 * Semelhante ao _.add do Lodash.
 *
 * @param value primeiro valor
 * @param other segundo valor
 * @returns soma (ou concatenação) dos dois valores
 */
export function add(value?: MaybeRefOrGetter<unknown>, other?: MaybeRefOrGetter<unknown>): number | string {
    ...
    if (b !== undefined) {
        ...
        if (typeof a === 'string' || typeof b === 'string') result = baseToString(a) + baseToString(b);
