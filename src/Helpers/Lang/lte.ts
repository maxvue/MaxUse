...
/**
 * Verifica se `value` é menor ou igual a `other`. Se ambos forem strings,
 * compara lexicograficamente; caso contrário, converte para número antes de
 * comparar.
 * Semelhante ao _.lte do Lodash.
 *
 * @param value valor a comparar
 * @param other valor de referência
 * @returns `true` se `value` for menor ou igual a `other`
 */
export function lte(value: MaybeRefOrGetter<unknown>, other: MaybeRefOrGetter<unknown>): boolean {
    let a = toValue(value);
    let b = toValue(other);
