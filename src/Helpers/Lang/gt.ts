...
/**
 * Verifica se `value` é maior que `other`. Se ambos forem strings, compara
 * lexicograficamente; caso contrário, converte para número antes de comparar.
 * Semelhante ao _.gt do Lodash.
 *
 * @param value valor a comparar
 * @param other valor de referência
 * @returns `true` se `value` for maior que `other`
 */
export function gt(value: MaybeRefOrGetter<unknown>, other: MaybeRefOrGetter<unknown>): boolean {
    let a = toValue(value);
    let b = toValue(other);
