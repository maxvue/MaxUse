...
/**
 * Verifica se `value` é menor que `other`. Se ambos forem strings, compara
 * lexicograficamente; caso contrário, converte para número antes de comparar.
 * Semelhante ao _.lt do Lodash.
 *
 * @param value valor a comparar
 * @param other valor de referência
 * @returns `true` se `value` for menor que `other`
 */
export function lt(value: MaybeRefOrGetter<unknown>, other: MaybeRefOrGetter<unknown>): boolean {
    let a = toValue(value);
    let b = toValue(other);
