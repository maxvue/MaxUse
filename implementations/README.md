...
| [018](./018-useDateFormat-fallback-mascara-erro.md) | `useDateFormat`/`timeAgo`: fallback para "hoje" mascara dado ausente e quebra reatividade | Composables | Mudar o fallback altera o que já é exibido em telas existentes |
...
"nunca acessou" é exibido como "acessou agora" — indistinguível de dado real. Há
também um sub-problema que **não** é decisão de produto e pode ser corrigido
isoladamente: quando o valor inicial é nulo, a reatividade é permanentemente
quebrada (o fallback passa um `Date` estático em vez do getter original), o que
atinge todo componente que carrega data via API.
