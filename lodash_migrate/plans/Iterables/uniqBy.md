...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/uniqBy.ts`
**Teste:** `src/Helpers/Iterables/uniqBy.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).uniqBy; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#uniqBy
...
consulte https://lodash.com/docs#uniqBy e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/uniqBy.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/uniqBy.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/uniqBy.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/uniqBy.test.ts` passa.
