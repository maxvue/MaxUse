...
**Categoria:** Iterables
**Fase:** 2
**Destino:** `src/Helpers/Iterables/concat.ts`
**Teste:** `src/Helpers/Iterables/concat.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).concat; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#concat
...
consulte https://lodash.com/docs#concat e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

não muta; achata 1 nível de arrays passados
...
2. Criar `src/Helpers/Iterables/concat.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/concat.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/concat.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/concat.test.ts` passa.
...
- [ ] Há teste dedicado para: não muta; achata 1 nível de arrays passados
