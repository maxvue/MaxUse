...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/map.ts`
**Teste:** `src/Helpers/Iterables/map.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).map; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#map
...
consulte https://lodash.com/docs#map e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

funciona em objeto; shorthand de string
...
2. Criar `src/Helpers/Iterables/map.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/map.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/map.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/map.test.ts` passa.
...
- [ ] Há teste dedicado para: funciona em objeto; shorthand de string
