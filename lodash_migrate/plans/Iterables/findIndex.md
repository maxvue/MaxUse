...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/findIndex.ts`
**Teste:** `src/Helpers/Iterables/findIndex.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).findIndex; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#findIndex
...
consulte https://lodash.com/docs#findIndex e determine o comportamento de
...
- [ ] `npx vitest run src/Helpers/Iterables/findIndex.test.ts` passa.
