...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/sortedIndexBy.ts`
**Teste:** `src/Helpers/Iterables/sortedIndexBy.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).sortedIndexBy; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#sortedIndexBy
...
consulte https://lodash.com/docs#sortedIndexBy e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/sortedIndexBy.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/sortedIndexBy.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/sortedIndexBy.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/sortedIndexBy.test.ts` passa.
