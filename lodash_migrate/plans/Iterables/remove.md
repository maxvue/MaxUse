...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/remove.ts`
**Teste:** `src/Helpers/Iterables/remove.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).remove; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#remove
...
consulte https://lodash.com/docs#remove e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

MUTA; retorna removidos
...
2. Criar `src/Helpers/Iterables/remove.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Iterables/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Iterables/remove.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/remove.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/remove.test.ts` passa.
...
- [ ] Há teste dedicado para: MUTA; retorna removidos
