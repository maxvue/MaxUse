...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/flatMapDepth.ts`
**Teste:** `src/Helpers/Iterables/flatMapDepth.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).flatMapDepth; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#flatMapDepth
...
consulte https://lodash.com/docs#flatMapDepth e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/flatMapDepth.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Iterables/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Iterables/flatMapDepth.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/flatMapDepth.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/flatMapDepth.test.ts` passa.
