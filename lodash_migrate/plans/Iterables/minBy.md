...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/minBy.ts`
**Teste:** `src/Helpers/Iterables/minBy.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).minBy; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#minBy
...
consulte https://lodash.com/docs#minBy e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/minBy.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/minBy.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/minBy.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/minBy.test.ts` passa.
