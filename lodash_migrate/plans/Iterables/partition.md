...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/partition.ts`
**Teste:** `src/Helpers/Iterables/partition.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).partition; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#partition
...
consulte https://lodash.com/docs#partition e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/partition.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/partition.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/partition.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/partition.test.ts` passa.
