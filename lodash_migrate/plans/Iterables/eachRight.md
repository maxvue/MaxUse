...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/eachRight.ts`
**Teste:** `src/Helpers/Iterables/eachRight.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).eachRight; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#eachRight
...
consulte https://lodash.com/docs#eachRight e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Alias

Este helper é um alias de `forEachRight`. A implementação deve re-exportar o original, não duplicar a lógica:

```typescript
export { forEachRight as eachRight } from './forEachRight';
```
...
2. Criar `src/Helpers/Iterables/eachRight.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/eachRight.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/eachRight.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/eachRight.test.ts` passa.
