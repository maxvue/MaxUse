...
**Categoria:** Iterables
**Fase:** 2
**Destino:** `src/Helpers/Iterables/fromPairs.ts`
**Teste:** `src/Helpers/Iterables/fromPairs.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).fromPairs; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#fromPairs
...
consulte https://lodash.com/docs#fromPairs e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/fromPairs.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Iterables/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Iterables/fromPairs.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/fromPairs.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/fromPairs.test.ts` passa.
