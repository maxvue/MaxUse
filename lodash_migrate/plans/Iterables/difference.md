...
**Categoria:** Iterables
**Fase:** 2
**Destino:** `src/Helpers/Iterables/difference.ts`
**Teste:** `src/Helpers/Iterables/difference.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).difference; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#difference
...
consulte https://lodash.com/docs#difference e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/difference.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/difference.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/difference.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/difference.test.ts` passa.
...
- [ ] Há teste dedicado para: SameValueZero
