...
**Categoria:** Iterables
**Fase:** 2
**Destino:** `src/Helpers/Iterables/compact.ts`
**Teste:** `src/Helpers/Iterables/compact.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).compact; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#compact
...
consulte https://lodash.com/docs#compact e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/compact.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/compact.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/compact.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/compact.test.ts` passa.
...
- [ ] Há teste dedicado para: remove false, null, 0, "", undefined, NaN
