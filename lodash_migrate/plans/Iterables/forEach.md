...
**Categoria:** Iterables
**Fase:** 3
**Destino:** `src/Helpers/Iterables/forEach.ts`
**Teste:** `src/Helpers/Iterables/forEach.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).forEach; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#forEach
...
consulte https://lodash.com/docs#forEach e determine o comportamento de
...
2. Criar `src/Helpers/Iterables/forEach.ts` seguindo o contrato do `CONVENTIONS.md`.
...
4. Criar `src/Helpers/Iterables/forEach.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/forEach.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/forEach.test.ts` passa.
...
- [ ] Há teste dedicado para: return false interrompe a iteração
