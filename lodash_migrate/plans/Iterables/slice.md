...
**Categoria:** Iterables
**Fase:** 2
**Destino:** `src/Helpers/Iterables/slice.ts`
**Teste:** `src/Helpers/Iterables/slice.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).slice; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#slice
...
consulte https://lodash.com/docs#slice e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

versão lodash não é a nativa
...
2. Criar `src/Helpers/Iterables/slice.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Iterables/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Iterables/slice.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Iterables/slice.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Iterables/slice.test.ts` passa.
...
- [ ] Há teste dedicado para: versão lodash não é a nativa
