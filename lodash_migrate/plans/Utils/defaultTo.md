...
**Categoria:** Utils
**Fase:** 1
**Destino:** `src/Helpers/Utils/defaultTo.ts`
**Teste:** `src/Helpers/Utils/defaultTo.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).defaultTo; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#defaultTo
...
consulte https://lodash.com/docs#defaultTo e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

substitui NaN, null e undefined
...
2. Criar `src/Helpers/Utils/defaultTo.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Utils/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Utils/defaultTo.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Utils/defaultTo.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Utils/defaultTo.test.ts` passa.
...
- [ ] Há teste dedicado para: substitui NaN, null e undefined
