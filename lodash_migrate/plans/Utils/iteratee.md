...
**Categoria:** Utils
**Fase:** 3
**Destino:** `src/Helpers/Utils/iteratee.ts`
**Teste:** `src/Helpers/Utils/iteratee.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).iteratee; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#iteratee
...
consulte https://lodash.com/docs#iteratee e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

string=>property, array=>matchesProperty, objeto=>matches, fn=>fn
...
2. Criar `src/Helpers/Utils/iteratee.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Utils/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Utils/iteratee.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Utils/iteratee.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Utils/iteratee.test.ts` passa.
...
- [ ] Há teste dedicado para: string=>property, array=>matchesProperty, objeto=>matches, fn=>fn
