...
**Categoria:** Utils
**Fase:** 4
**Destino:** `src/Helpers/Utils/templateSettings.ts`
**Teste:** `src/Helpers/Utils/templateSettings.test.ts`
**Registro:** `src/Helpers/Utils/index.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).templateSettings; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#templateSettings
...
consulte https://lodash.com/docs#templateSettings e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

objeto de config com escape/evaluate/interpolate
...
2. Criar `src/Helpers/Utils/templateSettings.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Utils/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Utils/templateSettings.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Utils/templateSettings.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Utils/templateSettings.test.ts` passa.
...
- [ ] Exportado em `src/Helpers/Utils/index.ts` (export plano **e** entrada no objeto namespace — ambos obrigatórios nesta categoria).
- [ ] `npm run lint` e `npm run type-check` passam.
- [ ] Há teste dedicado para: objeto de config com escape/evaluate/interpolate
