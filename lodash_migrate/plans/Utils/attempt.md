...
**Categoria:** Utils
**Fase:** 3
**Destino:** `src/Helpers/Utils/attempt.ts`
**Teste:** `src/Helpers/Utils/attempt.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).attempt; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#attempt
...
consulte https://lodash.com/docs#attempt e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

retorna o Error em vez de lançar
...
2. Criar `src/Helpers/Utils/attempt.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Utils/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Utils/attempt.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Utils/attempt.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Utils/attempt.test.ts` passa.
...
- [ ] Há teste dedicado para: retorna o Error em vez de lançar
