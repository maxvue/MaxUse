...
**Categoria:** Utils
**Fase:** 3
**Destino:** `src/Helpers/Utils/conformsTo.ts`
**Teste:** `src/Helpers/Utils/conformsTo.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).conformsTo; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#conformsTo
...
consulte https://lodash.com/docs#conformsTo e determine o comportamento de
...
2. Criar `src/Helpers/Utils/conformsTo.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Utils/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Utils/conformsTo.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Utils/conformsTo.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Utils/conformsTo.test.ts` passa.
