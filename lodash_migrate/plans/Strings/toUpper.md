...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/toUpper.ts`
**Teste:** `src/Helpers/Strings/toUpper.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toUpper; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toUpper
...
consulte https://lodash.com/docs#toUpper e determine o comportamento de
...
2. Criar `src/Helpers/Strings/toUpper.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Strings/toUpper.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/toUpper.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/toUpper.test.ts` passa.
...
- [ ] Há teste dedicado para: converte null => ""
