...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/pad.ts`
**Teste:** `src/Helpers/Strings/pad.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).pad; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#pad
...
consulte https://lodash.com/docs#pad e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

distribui sobra à direita
...
2. Criar `src/Helpers/Strings/pad.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Strings/pad.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/pad.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/pad.test.ts` passa.
...
- [ ] Há teste dedicado para: distribui sobra à direita
