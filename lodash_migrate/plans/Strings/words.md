...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/words.ts`
**Teste:** `src/Helpers/Strings/words.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).words; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#words
...
consulte https://lodash.com/docs#words e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

separa camelCase e unicode; aceita pattern
...
2. Criar `src/Helpers/Strings/words.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Strings/words.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/words.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/words.test.ts` passa.
...
- [ ] Há teste dedicado para: separa camelCase e unicode; aceita pattern
