...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/split.ts`
**Teste:** `src/Helpers/Strings/split.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).split; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#split
...
consulte https://lodash.com/docs#split e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

assinatura lodash com limit
...
2. Criar `src/Helpers/Strings/split.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Strings/split.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/split.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/split.test.ts` passa.
...
- [ ] Há teste dedicado para: assinatura lodash com limit
