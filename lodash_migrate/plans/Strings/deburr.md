...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/deburr.ts`
**Teste:** `src/Helpers/Strings/deburr.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).deburr; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```
...
consulte https://lodash.com/docs#deburr e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

remove diacríticos: "déjà vu" => "deja vu"
...
2. Criar `src/Helpers/Strings/deburr.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Strings/deburr.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/deburr.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/deburr.test.ts` passa.
...
- [ ] Há teste dedicado para: remove diacríticos: "déjà vu" => "deja vu"
