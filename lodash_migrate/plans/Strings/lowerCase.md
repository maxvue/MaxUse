...
**Categoria:** Strings
**Fase:** 3
**Destino:** `src/Helpers/Strings/lowerCase.ts`
**Teste:** `src/Helpers/Strings/lowerCase.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).lowerCase; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#lowerCase
...
consulte https://lodash.com/docs#lowerCase e determine o comportamento de
...
2. Criar `src/Helpers/Strings/lowerCase.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Strings/lowerCase.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/lowerCase.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/lowerCase.test.ts` passa.
...
- [ ] Há teste dedicado para: espaço como separador
