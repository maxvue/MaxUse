...
**Categoria:** Strings
**Fase:** 1
**Destino:** `src/Helpers/Strings/toLower.ts`
**Teste:** `src/Helpers/Strings/toLower.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toLower; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toLower
...
consulte https://lodash.com/docs#toLower e determine o comportamento de
...
2. Criar `src/Helpers/Strings/toLower.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Strings/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Strings/toLower.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Strings/toLower.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Strings/toLower.test.ts` passa.
