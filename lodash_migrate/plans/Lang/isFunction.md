...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/isFunction.ts`
**Teste:** `src/Helpers/Lang/isFunction.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).isFunction; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#isFunction
...
consulte https://lodash.com/docs#isFunction e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

true para async/generator/proxy de função
...
2. Criar `src/Helpers/Lang/isFunction.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/isFunction.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/isFunction.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/isFunction.test.ts` passa.
...
- [ ] Há teste dedicado para: true para async/generator/proxy de função
