...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/clone.ts`
**Teste:** `src/Helpers/Lang/clone.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).clone; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#clone
...
consulte https://lodash.com/docs#clone e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

shallow; preserva tipo (Date, RegExp, Map...)
...
2. Criar `src/Helpers/Lang/clone.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/clone.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/clone.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/clone.test.ts` passa.
...
- [ ] Há teste dedicado para: shallow; preserva tipo (Date, RegExp, Map...)
