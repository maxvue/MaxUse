...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/toPlainObject.ts`
**Teste:** `src/Helpers/Lang/toPlainObject.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toPlainObject; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toPlainObject
...
consulte https://lodash.com/docs#toPlainObject e determine o comportamento de
...
2. Criar `src/Helpers/Lang/toPlainObject.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/toPlainObject.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/toPlainObject.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/toPlainObject.test.ts` passa.
...
- [ ] Há teste dedicado para: copia propriedades herdadas
