...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/isElement.ts`
**Teste:** `src/Helpers/Lang/isElement.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).isElement; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#isElement
...
consulte https://lodash.com/docs#isElement e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

nodeType === 1 e não plain object
...
2. Criar `src/Helpers/Lang/isElement.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/isElement.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/isElement.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/isElement.test.ts` passa.
...
- [ ] Há teste dedicado para: nodeType === 1 e não plain object
