...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/isMap.ts`
**Teste:** `src/Helpers/Lang/isMap.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).isMap; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#isMap
...
consulte https://lodash.com/docs#isMap e determine o comportamento de
...
2. Criar `src/Helpers/Lang/isMap.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/isMap.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/isMap.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/isMap.test.ts` passa.
