...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/toFinite.ts`
**Teste:** `src/Helpers/Lang/toFinite.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toFinite; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toFinite
...
consulte https://lodash.com/docs#toFinite e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

Infinity vira 1.7976931348623157e+308
...
2. Criar `src/Helpers/Lang/toFinite.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/toFinite.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/toFinite.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/toFinite.test.ts` passa.
...
- [ ] Há teste dedicado para: Infinity vira 1.7976931348623157e+308
