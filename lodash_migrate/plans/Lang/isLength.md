...
**Categoria:** Lang
**Fase:** 1
**Destino:** `src/Helpers/Lang/isLength.ts`
**Teste:** `src/Helpers/Lang/isLength.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).isLength; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#isLength
...
consulte https://lodash.com/docs#isLength e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

inteiro >=0 e <= MAX_SAFE_INTEGER
...
2. Criar `src/Helpers/Lang/isLength.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Lang/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Lang/isLength.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Lang/isLength.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Lang/isLength.test.ts` passa.
...
- [ ] Há teste dedicado para: inteiro >=0 e <= MAX_SAFE_INTEGER
