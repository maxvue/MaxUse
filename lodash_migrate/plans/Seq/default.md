...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/default.ts`
**Teste:** `src/Helpers/Seq/default.test.ts`
...
```bash
node -e "const _=require('lodash'); console.log(_.default.toString())"
```

A documentação oficial está em https://lodash.com/docs#default
...
Este helper é um alias de `lodash`. A implementação deve re-exportar o original, não duplicar a lógica:

```typescript
export { lodash as default } from './lodash';
```
...
2. Criar `src/Helpers/Seq/default.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/default.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/default.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/default.test.ts` passa.
...
- [ ] Há teste dedicado para: export default do lodash-es
