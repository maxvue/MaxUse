...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/thru.ts`
**Teste:** `src/Helpers/Seq/thru.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).thru; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#thru
...
consulte https://lodash.com/docs#thru e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

retorna o resultado do interceptor
...
2. Criar `src/Helpers/Seq/thru.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/thru.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/thru.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/thru.test.ts` passa.
...
- [ ] Há teste dedicado para: retorna o resultado do interceptor
