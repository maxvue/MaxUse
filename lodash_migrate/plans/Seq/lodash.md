...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/lodash.ts`
**Teste:** `src/Helpers/Seq/lodash.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).lodash; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#lodash
...
consulte https://lodash.com/docs#lodash e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

a própria função _ chamável
...
2. Criar `src/Helpers/Seq/lodash.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/lodash.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/lodash.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/lodash.test.ts` passa.
...
- [ ] Há teste dedicado para: a própria função _ chamável
