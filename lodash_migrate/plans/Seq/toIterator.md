...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/toIterator.ts`
**Teste:** `src/Helpers/Seq/toIterator.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toIterator; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toIterator
...
consulte https://lodash.com/docs#toIterator e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Alias

Este helper é um alias de `wrapperToIterator`. A implementação deve re-exportar o original, não duplicar a lógica:
...
2. Criar `src/Helpers/Seq/toIterator.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/toIterator.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/toIterator.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/toIterator.test.ts` passa.
