...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/next.ts`
**Teste:** `src/Helpers/Seq/next.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).next; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#next
...
consulte https://lodash.com/docs#next e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Alias

Este helper é um alias de `wrapperNext`. A implementação deve re-exportar o original, não duplicar a lógica:
...
2. Criar `src/Helpers/Seq/next.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/next.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/next.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/next.test.ts` passa.
