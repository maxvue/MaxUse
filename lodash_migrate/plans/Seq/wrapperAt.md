...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/wrapperAt.ts`
**Teste:** `src/Helpers/Seq/wrapperAt.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).wrapperAt; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#wrapperAt
...
consulte https://lodash.com/docs#wrapperAt e determine o comportamento de
...
2. Criar `src/Helpers/Seq/wrapperAt.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/wrapperAt.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/wrapperAt.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/wrapperAt.test.ts` passa.
