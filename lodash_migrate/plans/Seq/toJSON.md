...
**Categoria:** Seq
**Fase:** 5
**Destino:** `src/Helpers/Seq/toJSON.ts`
**Teste:** `src/Helpers/Seq/toJSON.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toJSON; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toJSON
...
consulte https://lodash.com/docs#toJSON e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Alias

Este helper é um alias de `wrapperValue`. A implementação deve re-exportar o original, não duplicar a lógica:

```typescript
export { wrapperValue as toJSON } from './wrapperValue';
```
...
2. Criar `src/Helpers/Seq/toJSON.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Seq/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Seq/toJSON.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Seq/toJSON.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Seq/toJSON.test.ts` passa.
...
- [ ] Exportado em `src/Helpers/Seq/index.ts` (export plano **e** entrada no objeto namespace — ambos obrigatórios nesta categoria).
