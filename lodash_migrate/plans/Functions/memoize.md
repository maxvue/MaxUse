...
**Categoria:** Functions
**Fase:** 4
**Destino:** `src/Helpers/Functions/memoize.ts`
**Teste:** `src/Helpers/Functions/memoize.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).memoize; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#memoize
...
consulte https://lodash.com/docs#memoize e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

expõe .cache; resolver opcional
...
2. Criar `src/Helpers/Functions/memoize.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Functions/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Functions/memoize.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Functions/memoize.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Functions/memoize.test.ts` passa.
