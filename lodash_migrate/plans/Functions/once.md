...
**Categoria:** Functions
**Fase:** 4
**Destino:** `src/Helpers/Functions/once.ts`
**Teste:** `src/Helpers/Functions/once.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).once; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#once
...
consulte https://lodash.com/docs#once e determine o comportamento de
...
2. Criar `src/Helpers/Functions/once.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Functions/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Functions/once.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Functions/once.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Functions/once.test.ts` passa.
