...
**Categoria:** Functions
**Fase:** 4
**Destino:** `src/Helpers/Functions/debounce.ts`
**Teste:** `src/Helpers/Functions/debounce.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).debounce; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#debounce
...
consulte https://lodash.com/docs#debounce e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

leading/trailing/maxWait + cancel/flush/pending
...
2. Criar `src/Helpers/Functions/debounce.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Functions/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Functions/debounce.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Functions/debounce.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Functions/debounce.test.ts` passa.
...
- [ ] Há teste dedicado para: leading/trailing/maxWait + cancel/flush/pending
