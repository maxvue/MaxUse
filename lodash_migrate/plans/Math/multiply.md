...
**Categoria:** Math
**Fase:** 1
**Destino:** `src/Helpers/Math/multiply.ts`
**Teste:** `src/Helpers/Math/multiply.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).multiply; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#multiply
...
consulte https://lodash.com/docs#multiply e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

multiply() sem args retorna 1
...
2. Criar `src/Helpers/Math/multiply.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Math/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Math/multiply.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Math/multiply.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Math/multiply.test.ts` passa.
...
- [ ] Há teste dedicado para: multiply() sem args retorna 1
