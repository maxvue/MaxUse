...
**Categoria:** Math
**Fase:** 1
**Destino:** `src/Helpers/Math/ceil.ts`
**Teste:** `src/Helpers/Math/ceil.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).ceil; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#ceil
...
consulte https://lodash.com/docs#ceil e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

precisão: ceil(4.006, 2) => 4.01
...
2. Criar `src/Helpers/Math/ceil.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Math/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Math/ceil.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Math/ceil.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Math/ceil.test.ts` passa.
...
- [ ] Há teste dedicado para: precisão: ceil(4.006, 2) => 4.01
