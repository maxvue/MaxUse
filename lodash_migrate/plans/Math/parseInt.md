...
**Categoria:** Math
**Fase:** 1
**Destino:** `src/Helpers/Math/parseInt.ts`
**Teste:** `src/Helpers/Math/parseInt.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).parseInt; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#parseInt
...
consulte https://lodash.com/docs#parseInt e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

radix default 10 mesmo com "0x"
...
2. Criar `src/Helpers/Math/parseInt.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Math/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Math/parseInt.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Math/parseInt.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Math/parseInt.test.ts` passa.
...
- [ ] Exportado em `src/Helpers/Math/index.ts` (export plano — esta categoria **não tem** objeto namespace; export plano sozinho já satisfaz o registro, não crie um namespace novo).
- [ ] `npm run lint` e `npm run type-check` passam.
- [ ] Há teste dedicado para: radix default 10 mesmo com "0x"
