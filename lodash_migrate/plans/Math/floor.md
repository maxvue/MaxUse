...
**Categoria:** Math
**Fase:** 1
**Destino:** `src/Helpers/Math/floor.ts`
**Teste:** `src/Helpers/Math/floor.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).floor; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#floor
...
consulte https://lodash.com/docs#floor e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

precisão: floor(4.006, 2) => 4
...
2. Criar `src/Helpers/Math/floor.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Math/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Math/floor.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Math/floor.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Math/floor.test.ts` passa.
...
- [ ] Há teste dedicado para: precisão: floor(4.006, 2) => 4
