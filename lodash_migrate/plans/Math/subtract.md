...
**Categoria:** Math
**Fase:** 1
**Destino:** `src/Helpers/Math/subtract.ts`
**Teste:** `src/Helpers/Math/subtract.test.ts`
...
2. Criar `src/Helpers/Math/subtract.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Math/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Math/subtract.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Math/subtract.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Math/subtract.test.ts` passa.
