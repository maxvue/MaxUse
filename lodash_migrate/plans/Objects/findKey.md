...
**Categoria:** Objects
**Fase:** 3
**Destino:** `src/Helpers/Objects/findKey.ts`
**Teste:** `src/Helpers/Objects/findKey.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).findKey; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#findKey
...
consulte https://lodash.com/docs#findKey e determine o comportamento de
...
2. Criar `src/Helpers/Objects/findKey.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Objects/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Objects/findKey.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Objects/findKey.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Objects/findKey.test.ts` passa.
