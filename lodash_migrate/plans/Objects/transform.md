...
**Categoria:** Objects
**Fase:** 3
**Destino:** `src/Helpers/Objects/transform.ts`
**Teste:** `src/Helpers/Objects/transform.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).transform; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#transform
...
consulte https://lodash.com/docs#transform e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Peculiaridade do Lodash

accumulator inferido do tipo (isArray/isObject já são própria da MaxUse)
...
2. Criar `src/Helpers/Objects/transform.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Objects/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Objects/transform.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Objects/transform.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Objects/transform.test.ts` passa.
...
- [ ] Há teste dedicado para: accumulator inferido do tipo (isArray/isObject já são própria da MaxUse)
