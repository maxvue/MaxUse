...
**Categoria:** Objects
**Fase:** 3
**Destino:** `src/Helpers/Objects/entries.ts`
**Teste:** `src/Helpers/Objects/entries.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).entries; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#entries
...
consulte https://lodash.com/docs#entries e determine o comportamento de
forma empírica, chamando a função com entradas reais e observando o
resultado.

## Alias

Este helper é um alias de `toPairs`. A implementação deve re-exportar o original, não duplicar a lógica:

```typescript
export { toPairs as entries } from './toPairs';
```
...
2. Criar `src/Helpers/Objects/entries.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Objects/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Objects/entries.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Objects/entries.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Objects/entries.test.ts` passa.
