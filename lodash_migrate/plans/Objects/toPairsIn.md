...
**Categoria:** Objects
**Fase:** 2
**Destino:** `src/Helpers/Objects/toPairsIn.ts`
**Teste:** `src/Helpers/Objects/toPairsIn.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).toPairsIn; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#toPairsIn
...
consulte https://lodash.com/docs#toPairsIn e determine o comportamento de
...
2. Criar `src/Helpers/Objects/toPairsIn.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Objects/index.ts` (re-export flat **e** entrada no objeto namespace).
4. Criar `src/Helpers/Objects/toPairsIn.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Objects/toPairsIn.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Objects/toPairsIn.test.ts` passa.
