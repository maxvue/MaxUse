...
**Categoria:** Objects
**Fase:** 3
**Destino:** `src/Helpers/Objects/defaultsDeep.ts`
**Teste:** `src/Helpers/Objects/defaultsDeep.test.ts`
...
```bash
npx tsx -e "import * as lodash from 'lodash-es'; import { inspect } from 'node:util'; const v = (lodash as any).defaultsDeep; console.log(typeof v === 'function' ? (v.toString() || '(corpo removido pelo build do lodash-es)') : inspect(v, { depth: 1 }));"
```

A documentação oficial está em https://lodash.com/docs#defaultsDeep
...
consulte https://lodash.com/docs#defaultsDeep e determine o comportamento de
...
2. Criar `src/Helpers/Objects/defaultsDeep.ts` seguindo o contrato do `CONVENTIONS.md`.
3. Registrar o export em `src/Helpers/Objects/index.ts` (re-export flat — esta categoria não tem objeto namespace, não crie um).
4. Criar `src/Helpers/Objects/defaultsDeep.test.ts` com a cobertura obrigatória descrita no `CONVENTIONS.md`.
5. Rodar `npx vitest run src/Helpers/Objects/defaultsDeep.test.ts` até passar.
...
- [ ] `npx vitest run src/Helpers/Objects/defaultsDeep.test.ts` passa.
